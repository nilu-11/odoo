...
class TestSlotDatetimeCompute(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        # Template starts on Mon 2026-02-02
    ...
    def test_slot_on_monday_aligns_with_template_start(self):
        """A Monday slot whose template starts Mon 2026-02-02 → datetimes on that exact day."""
        ...
        self.assertEqual(slot.start_datetime, datetime(2026, 2, 2, 8, 0, 0))
        self.assertEqual(slot.end_datetime, datetime(2026, 2, 2, 8, 45, 0))

    def test_slot_on_wednesday_projects_forward(self):
        """A Wednesday slot from a Monday-starting template → datetimes on 2026-02-04."""
        slot = self.Slot.create({
            'template_id': self.template.id,
            'period_id': self.period.id,
            'subject_id': self.subject.id,
        ...
        self.assertEqual(slot.start_datetime, datetime(2026, 2, 4, 8, 0, 0))
        self.assertEqual(slot.end_datetime, datetime(2026, 2, 4, 8, 45, 0))
